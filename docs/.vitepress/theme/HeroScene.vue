...
onMounted(async () => {
  ...
  function animate() {
    ...
    // organic glow drift — layered sines at different frequencies
    const gx = 50 + Math.sin(t * 0.7) * 20 + Math.sin(t * 1.3) * 10;
    const gy = 50 + Math.cos(t * 0.5) * 20 + Math.cos(t * 1.1) * 10;
    el.style.setProperty(
      "--glow-bg",
      `radial-gradient(circle at ${gx}% ${gy}%, #6dd57e 20%, #30b8d0 80%)`,
    );
